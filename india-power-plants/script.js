...
function initMap() {
  const indiaStateStyle = [
    ...
    { featureType: "administrative.country", elementType: "geometry", stylers: [{ visibility: "on" }, { color: "#000000" }] },
    { featureType: "administrative.province", elementType: "geometry", stylers: [{ visibility: "on" }, { color: "#444444" }] },
    { featureType: "administrative.province", elementType: "labels.text.fill", stylers: [{ visibility: "on" }, { color: "#000000" }] },
    { featureType: "administrative.province", elementType: "labels.text.stroke", stylers: [{ visibility: "on" }, { color: "#ffffff" }] }
  ];
  ...
  map.data.setStyle({ fillColor: 'transparent', strokeColor: '#888', strokeWeight: 1 });

  for (let i = 0; i < totalCount; i++) {
    const [lat, lon, shortName, fullName, color] = countries[i];
...
document.getElementById("game-input").addEventListener("keyup", function () {
  ...
  const [lat, lon, , fullName, color] = countries[idx];
  ...
  new google.maps.Marker({
    ...
    label: {
      ...
      fontSize: "14px",
      fontWeight: "bold",
      color: color || "#2E6F40" // default greenish
    },
    icon: {
...
document.getElementById("give-up-button").addEventListener("click", function () {
  ...
  for (let i = 0; i < totalCount; i++) {
    ...
    if (!guessedNames.has(name)) {
      ...
      const [lat, lon, , fullName, color] = countries[i];
      oldMarker.setMap(null);
